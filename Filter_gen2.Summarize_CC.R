...
library(data.table)
#Local
 output_path <- "D:/PM25_Imputed_Results/Imputed/Summary/GENOTYPE.Y/"
 info_path <- "C:/Users/Lilith Moss/Documents/MATH/RESEARCH/BMA_Project/#Real_Data/Imputed_Info_Scores/"

#HPC
#output_path <- "/home/pmd-01/chemenya/CHS/PM25/Imputed/Summary/CC/"
...
info.hw <- as.data.frame(fread(paste0(info_path,"hw.info.freq"),header=F))[,-1]
info.nhw <- as.data.frame(fread(paste0(info_path,"nhw.info.freq"),header=F))[,-1]
...
which.name <- "GENY_P" #Change according to method
which.col <- which(names(mdat.filtered)==which.name)
mdat.filtered$P <- as.numeric(levels(mdat.filtered[,which.col]))[mdat.filtered[,which.col]]
mdat.filtered$CHR <- as.numeric(mdat.filtered$CHR)
mdat.filtered$BP <- as.numeric(mdat.filtered$BP)
mdat.filtered2 <- mdat.filtered[mdat.filtered$P>-99,]
...
lambda2
   ...
   chr22 <-s mdat.filtered2[mdat.filtered2$CHR==22,]
