...
run.BMA.Mult <- function(dat, returnResults=NULL, output.P.only=NULL, DSL=NULL) {
  if(sum(dat$Y)!=0 & sum(dat$G)!=0 & sum(dat$E)!=0 & sum(dat$G)!=nrow(dat) & nrow(dat)>150){
    ...
    #New
    # Gamma1.1 <- Gamma1[c(11,16),c(11,16)]
    # Gamma2.2 <- Gamma2[c(10,15),c(10,15)]
    # 
    Gamma1.1 <- Gamma1[c(main+1,int+1),c(main+1,int+1)]
    Gamma2.2 <- Gamma2[c(main,int),c(main,int)]
    ...
    #New
    # beta.hat1 <- matrix(unlist(r3$posterior.bymodel$mean[1])[c(11,16)],ncol=1)
    # beta.hat2 <- matrix(unlist(r3$posterior.bymodel$mean[2])[c(10,15)],ncol=1)
...
run.BMA.Mult.hisp <- function(dat, returnResults=NULL, output.P.only=NULL, DSL=NULL) {
  if(sum(dat$Y)!=0 & sum(dat$G)!=0 & sum(dat$E)!=0 & sum(dat$G)!=nrow(dat) & nrow(dat)>150){
    ...
    Ebeta <- matrix(c(r3$posterior$mean[main],r3$posterior$mean[int]),ncol=1) #Global expected values of main and interaction effects
    Gamma1 <- matrix(r3$posterior.bymodel$var[[1]],ncol=14)
    Gamma2 <- matrix(r3$posterior.bymodel$var[[2]],ncol=13)
    ...
    Gamma1.1 <- Gamma1[c(main+1,int+1),c(main+1,int+1)]
    Gamma2.2 <- Gamma1[c(main,int),c(main,int)]
